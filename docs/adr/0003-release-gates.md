...
The refresh step now also regenerates `src/catalog/deals.ts` (tier, benchmarks, deal discounts with `was`/`now` rates, peak/off-peak windows, and GOAT/Pro allowances for every model) from the Command Code docs pages. The stale gate covers `deals.ts` alongside `snapshot.ts`/`facts.ts`; drift fails the run with instructions to run `npm run refresh:deals -- --fixtures` (offline, from `tests/fixtures/*.html`) or `npm run refresh:deals` (live fetch), commit, and re-tag. The TUI panel (`src/tui/index.tsx`) is compiled with the solid transform (`scripts/build-tui.ts` via `@opentui/solid/bun-plugin`, so `bun` is required on PATH for `npm run build`) — without it JSX props freeze and the sidebar never repaints on mid-session model switches.
