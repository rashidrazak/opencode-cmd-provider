...
## Amendment: deals catalog gate (2026-08-21)

The refresh step now also regenerates the Deals catalog (`src/deals/catalog.ts` — tier, benchmarks, deal discounts with `was`/`now` rates, peak/off-peak windows, and GOAT/Pro allowances for every model) from the Command Code docs' RSC stream (the docs pages fetched with an `rsc: 1` header — see ADR-0005; the HTML scrape is gone). Unlike `snapshot.ts`/`facts.ts`, the Deals gate is **non-blocking**: because the upstream RSC shape can change, a stale or empty Deals catalog emits a warning (not a failure) so a broken fetch cannot block Catalog refresh or Release. To update it, run `npm run refresh:deals` (live; 5xx/network falls back to the committed `tests/fixtures/rsc-*.txt` fixtures, 4xx fails loudly) or `npm run refresh:deals -- --fixtures` (offline), commit, and re-tag. The TUI panel (`src/deals/tui.tsx`) is compiled with the solid transform (`scripts/build-tui.ts` via `@opentui/solid/bun-plugin`, so `bun` is required on PATH for `npm run build`) — without it JSX props freeze and the sidebar never repaints on mid-session model switches. See ADR-0004 for the Deals intelligence slice boundary.
