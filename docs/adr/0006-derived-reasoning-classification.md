...
> **Status note (issue #132).** The "Snapshot model with no RSC record —
> shared coverage gate fails the refresh" contract recorded below is
> **inverted** since #132: classification is any-true-wins across models.md
> efforts, the RSC `reasoning` flag, and the models page Caps Reasoning bit;
> the capability map is sparse, and a model with no evidence anywhere ships
> in `MODEL_REASONING_PENDING` (behaves non-reasoning, flips with zero code
...
> _required_ slug-record field. The formal supersede-note and glossary
