...
2. **No account claim without a `whoami` answer.** The label comes from the
   ...
   elided `user.id` — never `user.email`, never a key). When that leg fails or
   names no user, the line renders `Credential: …` alone: an absent label, not a
   guessed one.
3. **A pin asks for no identity.** The `plan` argument and `COMMANDCODE_PLAN`
   ...
   to name an account.
4. **The label is inert.** It is markdown-flattened and truncated (32 chars)
   before rendering, so a value read from the API cannot forge a table row or
   break out of the line it sits in.
