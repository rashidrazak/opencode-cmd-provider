...
async function collect(model: Model, prompt: LanguageModelV3Prompt): Promise<Array<Record<string, unknown>>> {
...
run([
  ["streams text deltas and finish", async () => {
    ...
    try {
      ...
      const finish = parts.find((p) => p.type === "finish") as { finishReason?: unknown; usage?: unknown }
  ...
  ["streams reasoning deltas before text", async () => {
    const mock = await startMockCc({ stream: [reasoningDelta("think"), textDelta("answer"), finishEvent()] })
    try {
      const provider = createCommandCode({ apiKey: "user_test", baseURL: mock.url })
      const parts = await collect(provider.languageModel("claude-sonnet-5"), [{ role: "user", content: "hi" }])
  ...
  ["streams tool calls", async () => {
    const mock = await startMockCc({
      stream: [toolCall("t1", "read", { path: "a.ts" }), finishEvent({ finishReason: "tool_use" })],
    })
    try {
      const provider = createCommandCode({ apiKey: "user_test", baseURL: mock.url })
      const parts = await collect(provider.languageModel("claude-sonnet-5"), [{ role: "user", content: "hi" }])
  ...
  ["sends the CC payload shape", async () => {
  ...
  ["doStream surfaces a helpful no-key error", async () => {
    const mock = await startMockCc({ stream: [finishEvent()] })
    try {
      const model = createCommandCode({ baseURL: mock.url, authPaths: [] }).languageModel("claude-sonnet-5")
      const result = await model.doStream({ prompt: [{ role: "user", content: "hi" }], mode: { type: "regular" } })
      const reader = result.stream.getReader()
