...
run([
  ...
  ["parseStreamEventLine parses data JSON", () => {
    assertEqual(parseStreamEventLine('data: {"type":"text-delta","text":"hi"}'), { type: "text-delta", text: "hi" })
  }],
  ...
  ["mapFinishReason maps CC reasons to AI SDK v3 finish reasons", () => {
    ...
    assertEqual(mapFinishReason("max_output_tokens"), { unified: "length", raw: "max_output_tokens" })
  ...
  ["tool-call maps to v3 tool-input start/delta/end + tool-call parts", () => {
    const parts = ccEventToStreamPart({ type: "tool-call", toolCallId: "t1", toolName: "read", input: { path: "x" } })
    ...
    const call = parts[3] as { type: string; toolCallId?: string; toolName?: string; input?: string }
  ...
  ["finish with missing details falls back to total input", () => {
    const parts = ccEventToStreamPart({ type: "finish", finishReason: "stop", totalUsage: { inputTokens: 9, outputTokens: 2 } })
    const finish = parts[0] as { usage?: { inputTokens?: { total?: number }; outputTokens?: { total?: number } } }
    assertEqual(finish.usage, {
...
run([
  ["mock-cc streams SSE events and records hits", async () => {
  ...
  ["mock-cc models endpoint returns empty list by default", async () => {
