...
function usage(input: number, output: number, cacheRead: number, cacheWrite: number, cacheWrite1h = 0) {
  return {
    input,
    output,
    cacheRead,
...
run([
  ["flat rates are applied per million tokens", () => {
    ...
    calculateCommandCodeCost(
      { cost: MODEL_COSTS["deepseek/deepseek-v4-flash"] ?? { input: 0.14, output: 0.28, cacheRead: 0, cacheWrite: 0 } },
      u,
    )
