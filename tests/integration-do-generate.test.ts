...
run([
  ["doGenerate aggregates text + reasoning + usage", async () => {
    const mock = await startMockCc({
      stream: [reasoningDelta("think"), textDelta("Hello "), textDelta("world"), finishEvent()],
    })
    try {
      const model = createCommandCode({ apiKey: "user_test", baseURL: mock.url }).languageModel("claude-sonnet-5")
  ...
  ["doGenerate aggregates tool calls", async () => {
    const mock = await startMockCc({
    ...
    try {
      ...
      const calls = (result.content ?? []).filter((p) => (p as { type?: string }).type === "tool-call")
  ...
  ["doGenerate surfaces stream errors", async () => {
