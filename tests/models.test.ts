...
run([
  ...
  ["loads live catalog and caches it", async () => {
    const cacheDir = await mkdtemp(join(tmpdir(), "cc-models-"))
    const cachePath = join(cacheDir, "models.json")
    const mock = await startMockCc({
      models: { object: "list", data: [{ id: "claude-sonnet-5", name: "Claude Sonnet 5", context_length: 200000 }] },
    })
    try {
  ...
  ["falls back to cache when the endpoint is down", async () => {
    const cacheDir = await mkdtemp(join(tmpdir(), "cc-models-"))
    const cachePath = join(cacheDir, "models.json")
    const mock = await startMockCc({
      models: { object: "list", data: [{ id: "claude-sonnet-5", name: "Claude Sonnet 5", context_length: 200000 }] },
    })
    await loadCommandCodeModels({ url: `${mock.url}/provider/v1/models`, cachePath, timeoutMs: 1000 })
    await mock.close()
    const result = await loadCommandCodeModels({ url: `${mock.url}/provider/v1/models`, cachePath, timeoutMs: 1000 })
    assertEqual(result.source, "cache")
    assertEqual(result.models.length, 1)
    await rm(cacheDir, { recursive: true, force: true })
  }],

  ["empty result with warning when nothing works", async () => {
    const cacheDir = await mkdtemp(join(tmpdir(), "cc-models-"))
    const cachePath = join(cacheDir, "models.json")
    const mock = await startMockCc()
    try {
