...
function renderFacts({
  ...
  const modalityLines = Object.entries(modalities).map(
    ([id, values]) => `  ${JSON.stringify(id)}: ${JSON.stringify(values)},`,
  )
  return [
    ...
    `export const MODALITIES_SOURCE_URL = ${JSON.stringify(modalitiesSourceUrl)}`,
...
const snapshotIds = new Set(models.map((model) => model.id))
const missingModalities = models
  .filter((model) => !parsedModalities.modelIds.has(model.id))
  .map((model) => model.id)
if (missingModalities.length > 0) {
  fail(`CLI modality catalog is missing API models: ${missingModalities.join(", ")}`)
}
